...
```{r,message=FALSE,warning=FALSE}
#setwd("~/Desktop")
dataset <- rlms_read("r22i_os_32.sav")
```

```{r}
df <- dataset %>% select(rj13.2, rh6, rh5, r_diplom, status, rj1.1.1)
df <- mutate(df, age = 2013 - rh6)
df <- df %>% filter(status == 1 | status == 2)
df <- df %>% filter(rj1.1.1 == 1 | rj1.1.1 == 2 | is.na(rj1.1.1))
```

Подготовим данные к анализу и выберем переменные.Соберём наш прекрасный массив :)
```{r}
data <- mutate(df, 
               sex = as.numeric(rh5 == 1), 
               age = 2013 - rh6, 
               wage = rj13.2, 
               ...
               city = as.numeric(status == 2), 
               udovl = as.numeric(rj1.1.1 == 1)) %>% 
  dplyr::select(sex, age, wage, educ_l, educ_m, educ_ms, educ_h, city, udovl)
```
